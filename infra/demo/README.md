...
Latest GHCR deploy verification: GitHub Actions published `latest-demo` from `e5b2b0af9197`, but the local token used on 2026-05-15 could not read `ghcr.io/cyson21/stockrush-gateway:latest-demo` and GHCR returned `DENIED`. Re-run `./scripts/deploy-local.sh --login --tag latest-demo` after the token has package read permission or the package visibility is public.
