// CorrelationIdFilterIntegrationTest: 요청 공통 컨텍스트(상관관계 ID 등)를 가공해 추적 관점을 유지합니다.
...
class CorrelationIdFilterIntegrationTest {
    ...
    void stores_incoming_correlation_id_in_mdc_during_request_and_clears_it_afterwards() throws Exception {
        mockMvc.perform(get("/test/correlation-mdc")
                .header(CorrelationIds.HEADER_NAME, "corr-inventory-mdc"))
            .andExpect(status().isOk())
            .andExpect(header().string(CorrelationIds.HEADER_NAME, "corr-inventory-mdc"))
            .andExpect(jsonPath("$.success", is(true)))
            .andExpect(jsonPath("$.data", is("corr-inventory-mdc")))
            .andExpect(jsonPath("$.trace.correlationId", is("corr-inventory-mdc")));
