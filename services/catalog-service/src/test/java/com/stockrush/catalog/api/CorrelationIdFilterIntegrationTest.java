...
class CorrelationIdFilterIntegrationTest {
    ...
    void stores_incoming_correlation_id_in_mdc_during_request_and_clears_it_afterwards() throws Exception {
        mockMvc.perform(get("/test/correlation-mdc")
                .header(CorrelationIds.HEADER_NAME, "corr-catalog-mdc"))
            .andExpect(status().isOk())
            .andExpect(header().string(CorrelationIds.HEADER_NAME, "corr-catalog-mdc"))
            .andExpect(jsonPath("$.success", is(true)))
            .andExpect(jsonPath("$.data", is("corr-catalog-mdc")))
            .andExpect(jsonPath("$.trace.correlationId", is("corr-catalog-mdc")));
