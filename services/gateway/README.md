...
The routing smoke test uses fake upstream services and verifies method, path, query string, body, `Idempotency-Key`, `X-Correlation-Id`, response status, `Location`, and response body propagation for customer order routes, admin order routes, and Outbox admin routes.
