...
- Expo React Native 기반 Android/iOS 고객 모바일 앱 scaffold, Gateway-first API client, 상품/SKU 재고 조회, 쿠폰 견적, 주문 생성, 주문 상태 추적 화면
...
4. 모바일 앱은 `apps/mobile-app`에서 Expo scaffold, API client, 상품/SKU 재고, 쿠폰/주문 생성, 주문 상태 추적 화면을 확인합니다.
...
- Expo React Native 모바일 앱: Android/iOS에서 고객 주문 흐름과 Read Model 기반 주문 내역을 시연할 수 있도록 Gateway-first client 구조를 잡고, 상품/SKU 재고 조회, 쿠폰/주문 생성, 주문 상태 추적 화면을 연결했습니다.
...
- 모바일 앱은 상품/SKU 재고, 쿠폰 견적, 주문 생성, 주문 상태 추적 화면을 React Native Testing Library, Jest Expo, TypeScript typecheck, scaffold validation으로 검증했습니다. Android 또는 iOS smoke는 후속으로 진행합니다.
