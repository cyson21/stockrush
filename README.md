...
- 외부 진입점은 Gateway로 제한하고, 클라이언트 요청은 Gateway에서 JWT/역할 검증 후 내부로 전달됩니다.
- 내부 서비스는 클라이언트가 보낸 `memberId`·`operator`를 그대로 신뢰하지 않고, 소유권/권한 재검증 경로를 둡니다.
- 데모 compose와 Nginx 공개 경로는 Gateway-only 기준을 따르며, 내부 백엔드 서비스 host port 공개와 레거시 서비스 prefix proxy는 Architecture Guard로 막습니다.
- Gateway 경계와 내부 서비스 신뢰 경계를 분리해 설명하면 이력서 과장을 줄일 수 있습니다.
...
CI는 서비스 테스트, 웹/모바일 테스트, Architecture Guard, secret scan, Trivy filesystem scan을 실행합니다.

- `main` 기준 CI는 GitHub Actions 배지/Release Images workflow의 `main` 브랜치 실행 상태를 뜻합니다.
...
- 서비스-local 직접 호출은 host 개발 모드의 서비스 디버깅용으로만 남깁니다. 공개 포트폴리오·데모·smoke 진입점은 Gateway와 웹앱으로 제한합니다.
