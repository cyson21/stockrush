...
로컬 개발 모드에서 주문, 재고, 결제, 운영 복구 흐름을 재현하는 절차를 정리한다.
...
- Node.js 20 + npm
- `jq`
- `./scripts/with-java17.sh mvn -version`, `node -v`, `npm -v`, `jq --version`이 동작해야 함
- host 개발 모드에서는 포트 충돌이 없어야 함:
...
- 데모 모드: `infra/demo` Docker Compose로 인프라, 백엔드, 웹앱을 함께 실행한다. 데모 모드는 Gateway(`28080`), 웹앱(`15173~15174`), Keycloak(`28088`), 필수 인프라(`25432`, `26379`, `29092`, `29090`)만 host port로 공개한다. 내부 backend service는 compose network 안에 두고 Gateway로만 접근한다. macOS/Windows 11 실행법은 `infra/demo/README.md`에 둔다.
- 웹 화면 캡처와 시각 점검 절차는 [Web Visual Smoke Runbook](web-visual-smoke.md)에 둔다.
...
```bash
cd services/gateway && ../../scripts/with-java17.sh mvn spring-boot:run
cd services/catalog-service && ../../scripts/with-java17.sh mvn spring-boot:run
...
Promotion Service 고객 견적은 Gateway의 `/api/coupons/quote` route로 확인한다. Customer App은 `/api/products`, `/api/stocks`, `/api/coupons/quote`, `/api/orders` 모두 Gateway 경유 경로를 호출하고, Order Service는 내부 `PROMOTION_SERVICE_URL`로 quote API를 호출한다. `PROMOTION_KAFKA_LISTENERS_ENABLED=true`로 기동하면 주문 이벤트를 소비해 쿠폰 사용 상태를 `RESERVED`, `CONSUMED`, `RELEASED`로 기록한다.

Fulfillment Service는 현재 이벤트 기반 출고 준비 요청 기록을 담당한다. Read Model Service는 Gateway의 `/api/read-model/orders`, `/api/read-model/admin/orders` route로 주문 요약 projection을 제공한다.
...
- `--relay-mode manual`: runner가 Gateway Outbox retry API를 호출한다. 서비스별 retry/requeue 운영 API 확인에 사용한다.
- `--relay-mode automatic`: runner가 retry API를 호출하지 않고 서비스 scheduler가 outbox를 발행하기를 기다린다. 데모 스택 smoke 기본 기준이며, 사용자가 직접 retry 버튼을 누르지 않아도 주문이 수렴하는지 검증한다.

automatic 모드는 주문 생성, 결제, 주문 상태 반영, 관리자 취소, 재고 해제까지 여러 scheduler 주기를 통과한다. 데모 smoke는 짧은 polling으로 오탐이 나지 않도록 `--max-attempts 30 --wait-seconds 1`을 기본 예산으로 둔다.
...
장기 체류 `PENDING`/`FAILED` row는 Local E2E runner로 운영 API 복구 흐름을 반복 실행한다.
...
- Gateway 경유로 Order/Inventory/Payment outbox `PENDING,FAILED` row를 조회한다.
- 기본값은 `FAILED` row를 requeue한 뒤 `PENDING` retry를 실행한다.
- 실행 요청은 `X-Operator-Id`와 실행별 `X-Correlation-Id`를 전달해 admin action audit row에 추적값을 남긴다.
- `nextRetryAt`이 미래인 `PENDING` row는 deferred로 분류하고 실패로 보지 않는다.
- 최종 `after.retryablePendingCounts`와 `after.failedCounts`가 모두 `0`이어야 한다.

### Kafka Outage Recovery

Kafka broker 장애 주입은 기본 smoke와 분리된 opt-in 시나리오로 실행한다. 이 명령은 demo Docker Compose의 Kafka service를 `pause/unpause`하므로 다른 검증과 동시에 실행하지 않는다.
...
./tools/local-e2e/local-e2e kafka-outage-recovery \
  ...
  --kafka-service kafka \
  --order-api-url http://localhost:28080 \
  --admin-api-url http://localhost:28080 \
...
- Kafka pause 구간에서는 주문이 `CONFIRMED/COMPLETED` 또는 `CANCELLED/FAILED`로 바로 수렴하지 않아야 한다.
- pause 구간에서 이번 실행으로 증가한 Order/Inventory/Payment `PENDING`/`PUBLISHING`/`FAILED` outbox가 관측되어야 한다.
- Kafka unpause 이후 주문은 `CONFIRMED/COMPLETED`로 수렴해야 한다.
- 최종 재고는 `availableQuantity=initialStock-quantity`, `reservedQuantity=0`이어야 한다.
- 최종 Order/Inventory/Payment `pendingOutboxDelta`와 신규 `PENDING`/`PUBLISHING`/`FAILED` outbox event ID가 비어 있어야 한다.
...
최근 Kafka outage recovery 검증 증거:

- 실행 시각: 2026-05-15 01:35 KST
- productCode: `KAFKA-OUTAGE-E2E-20260515014056-84904d76`
- orderId: `ord_20260514164100_80a5f7f8`
- pause 구간: 주문 `CREATED/STARTED`, order outbox 1건 관측
- unpause 이후: 주문 `CONFIRMED/COMPLETED`
- 최종 재고: `availableQuantity=2`, `reservedQuantity=0`
- Order/Inventory/Payment `finalPendingOutboxDelta`: 모두 `0`
...
./tools/local-e2e/local-e2e demo-order-flow \
  ...
  --wait-seconds 1 \
  --relay-mode automatic \
  --order-api-url http://localhost:18080 \
  --admin-api-url http://localhost:18080 \
...
SKU_ID=DEMO-001-S
ADMIN_BEARER_TOKEN="$STOCKRUSH_ADMIN_BEARER_TOKEN"
CUSTOMER_BEARER_TOKEN="$STOCKRUSH_CUSTOMER_BEARER_TOKEN"

curl -sS -X POST http://localhost:18080/api/admin/products \
...
curl -sS -X PUT http://localhost:18080/api/admin/stocks/"$SKU_ID" \
  -H "Authorization: Bearer $ADMIN_BEARER_TOKEN" \
  -H 'Content-Type: application/json' \
...
CARD_ORDER_ID=$(curl -sS -X POST http://localhost:18080/api/orders \
  ...
  -d "{\"paymentMethod\":\"CARD\",\"items\":[{\"productCode\":\"$PRODUCT_CODE\",\"skuId\":\"$SKU_ID\",\"quantity\":1,\"unitPrice\":12000}]}" \
  | jq -r '.data.orderId')
...
FAIL_ORDER_ID=$(curl -sS -X POST http://localhost:18080/api/orders \
  ...
  -d "{\"paymentMethod\":\"FAIL_CARD\",\"items\":[{\"productCode\":\"$PRODUCT_CODE\",\"skuId\":\"$SKU_ID\",\"quantity\":1,\"unitPrice\":12000}]}" \
  | jq -r '.data.orderId')
...
```bash
curl -sS "http://localhost:18080/api/stocks?productCode=$PRODUCT_CODE"
```
...
DELAY_ORDER_ID=$(curl -sS -X POST http://localhost:18080/api/orders \
  ...
  -d "{\"paymentMethod\":\"DELAY_CARD\",\"items\":[{\"productCode\":\"$PRODUCT_CODE\",\"skuId\":\"$SKU_ID\",\"quantity\":1,\"unitPrice\":12000}]}" \
  | jq -r '.data.orderId')
...
- [ ] host 개발 모드에서 18080~18087 모든 서비스 `actuator/health` 응답 200
- [ ] host 개발 모드에서 18080~18087 모든 서비스 `actuator/info` 응답 200
- [ ] host 개발 모드에서 18080~18087 모든 서비스 `actuator/metrics` 응답 200
