...
- Local E2E: `CARD`, `FAIL_CARD`, `DELAY_CARD`, 지연 결제 취소 시나리오를 실제 로컬 서비스와 Kafka로 확인

문서에는 구현된 검증과 남은 공백을 분리했습니다. 현재 Gateway routing, 인증/권한, Kafka 장애 복구 자동화, 명시적 동시 주문 경합 자동 테스트는 후속 확장 범위로 남아 있습니다.
...
주문 요청의 SKU별 수량을 먼저 합산하고, 재고 row를 잠근 상태에서 조건부로 available 수량을 차감한다. 이 방식으로 같은 주문 안의 중복 SKU와 동시 주문 상황에서 음수 재고가 생길 가능성을 줄였다.
