...
겉으로는 React/Vite 고객 웹앱에서 상품 조회, 주문 생성, 주문 상태 추적을 할 수 있고, 관리자 웹앱에서는 상품/재고 관리, Saga 상태 확인, Outbox 재시도와 failed requeue, 지연 결제 취소를 확인할 수 있습니다. Expo React Native 기반 Android/iOS 고객 앱은 scaffold와 Gateway-first API client 구조를 추가했고, 같은 백엔드 흐름을 모바일에서도 시연할 수 있도록 화면 구현을 이어갈 계획입니다. 개발 과정에서는 Dev RAG, Project MCP, Spark worker/reviewer, Agent Runner, Architecture Guard를 활용해 AI 기반 개발 운영 흐름도 함께 구축했습니다.
...
운영 화면도 함께 만들었습니다. 현재 고객 웹앱은 상품 목록, 상품 선택, SKU 재고 확인, 주문 생성, 주문 상태 polling을 제공하고, 관리자 웹앱은 상품 등록/수정, SKU 재고 설정, 주문/Saga 추적, Outbox 조회/재시도/requeue, 지연 결제 취소를 제공합니다. 모바일 고객 앱은 Expo React Native scaffold와 Gateway-first API client를 갖췄고, 다음 단계에서 동일한 주문 흐름과 Read Model 기반 주문 내역을 Android/iOS 화면으로 연결합니다. 특히 `DELAY_CARD` 주문은 관리자가 취소 요청을 보내면 `PaymentCancelRequested` command와 `PaymentCanceled` 이벤트를 거쳐 주문 취소와 재고 복구로 이어지게 했습니다.
...
| fulfillment-service | 주문 완료 이후 출고 준비 요청 기록 |
...
고객 앱은 포트폴리오 시연에서 사용자가 흐름을 이해하기 위한 진입점입니다. 상품을 선택하고 SKU 재고와 쿠폰 할인 견적을 확인한 뒤 `CARD`, `FAIL_CARD`, `DELAY_CARD` 결제수단으로 주문을 생성할 수 있습니다. 주문 생성 후에는 주문 조회 API를 polling해 상태 변화를 보여줍니다.

관리자 앱은 운영 복구 경로를 보여줍니다. 상품 등록/수정, SKU 재고 설정, 주문 목록과 Saga 상세 조회, 서비스별 Outbox 조회/재시도/requeue, 지연 결제 취소 요청을 한 화면 흐름으로 연결했습니다.
...
- Mobile: 구현 후 Expo 실행, React Native 화면 테스트, Android/iOS 중 최소 한 환경 smoke로 고객 주문 흐름 검증
- Gateway: 주문 생성/조회, 관리자 주문 조회/취소, Outbox 조회/재시도/requeue 라우팅 smoke로 path, query, header, body, status 전달을 검증
- Promotion/Order pricing: 쿠폰 생성/목록/견적, 주문 할인 반영, Payment command 결제 예정 금액, 쿠폰 사용 상태 전이를 검증
- Fulfillment: 주문 완료 이벤트 소비, 출고 준비 요청 생성, 중복 이벤트 처리를 검증
- Read Model: 주문 생성/완료/취소 이벤트 projection, JSON consumer dispatch, 고객/관리자 주문 요약 API를 검증
...
문서에는 구현된 검증과 남은 공백을 분리했습니다. 현재 동일 SKU runner와 runbook의 주문 생성/조회/취소 및 Outbox 운영 경로는 Gateway를 경유합니다. Outbox retry/requeue는 서비스별 감사 로그를 남깁니다. Promotion은 고객 앱 견적 UI, 주문 할인 반영, 주문 이벤트 기반 쿠폰 사용 상태까지 연결했고, Fulfillment는 주문 완료 이벤트 기반 출고 준비 요청까지 연결했습니다. Read Model은 주문 요약 projection과 서비스-local 고객/관리자 API까지 연결했습니다. Promotion/Fulfillment/Read Model Gateway route, 관리자 사용 이력/출고 화면, 인증/권한, Kafka 장애 복구 자동화, 부하 벤치마크와 Kafka consumer 병렬성 검증은 후속 확장 범위입니다.
...
- Promotion Service: Gateway route, 관리자 쿠폰 사용 이력 화면
- Fulfillment Service: carrier/label/tracking 상태와 관리자 출고 화면
- Read Model: Gateway route, 상품 검색 projection, 관리자 대시보드
- Mobile App: Expo React Native 고객 앱, Android/iOS smoke, 모바일 주문 내역 화면
...
- 고객 앱과 관리자 앱을 함께 구현해 상품 조회, 주문 생성, 주문 상태 추적, 상품/재고 관리, Outbox retry/requeue, 지연 결제 취소 운영 흐름을 시연 가능하게 구성
- Expo React Native 기반 Android/iOS 고객 앱을 후속 확장으로 계획해 동일한 Kafka/Saga/Read Model 백엔드 흐름을 모바일 UX로 연결하도록 설계
...
- Fulfillment Service가 `OrderConfirmed`를 소비해 출고 준비 요청을 기록하고 중복 이벤트를 무해하게 처리하도록 구현
- Read Model Service가 주문 lifecycle event를 소비해 고객 주문 내역과 관리자 주문 요약 projection을 갱신하도록 구현
...
인증/권한, 관측성, 장애 복구 자동화, 부하/consumer 병렬성 검증은 아직 후속 범위다. 현재 버전은 커머스 핵심 Saga와 Gateway 경유 운영 복구 흐름, 그리고 최소 운영 감사 기록을 설명 가능한 수준으로 연결하는 데 집중했다.
