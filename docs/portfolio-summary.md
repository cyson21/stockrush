...
- Security: 고객 object-level authorization, 앱 OIDC login, 인증 주체 기반 admin audit
...
Gateway 관리자 인증/권한은 OAuth2 Resource Server 기반으로 시작했고, demo Keycloak realm과 smoke token 취득 흐름까지 연결했다. 고객 object-level authorization, 앱 OIDC login, 인증 주체 기반 admin audit, 관측성, 부하/consumer 병렬성 검증은 아직 후속 범위다. 현재 버전은 커머스 핵심 Saga와 Gateway 경유 운영 복구 흐름, Kafka 장애 복구 smoke, 그리고 최소 운영 감사 기록을 설명 가능한 수준으로 연결하는 데 집중했다.
