...
- Promotion: 쿠폰 생성/목록/견적 통합 테스트로 할인 계산과 오류 응답을 검증
...
문서에는 구현된 검증과 남은 공백을 분리했습니다. 현재 동일 SKU runner와 runbook의 주문 생성/조회/취소 및 Outbox 운영 경로는 Gateway를 경유합니다. Outbox retry/requeue는 서비스별 감사 로그를 남깁니다. Promotion은 아직 Gateway/UI/주문 할인 반영과 연결하지 않은 서비스-local 범위입니다. 인증/권한, Kafka 장애 복구 자동화, 부하 벤치마크와 Kafka consumer 병렬성 검증은 후속 확장 범위로 남아 있습니다.
...
- Promotion Service: 주문 생성 할인 반영, 쿠폰 사용/복구 이벤트
