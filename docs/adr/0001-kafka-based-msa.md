...
StockRush는 한정 판매 커머스 주문 흐름을 백엔드 중심으로 구현하는 프로젝트다.

단순 CRUD 또는 단일 서버 구조만으로는 서비스 분리 이후의 일관성, 장애 복구, 운영 가시성 문제를 충분히 다루기 어렵다. 커머스 도메인에서는 주문, 재고, 결제, 프로모션이 서로 다른 변경 이유와 정합성 요구를 가진다.
...
- Kafka, Saga, Outbox, 보상 처리 경로를 다루기 어렵다.
- 분산 주문 흐름에서 다룰 수 있는 실패 지점이 제한적이다.

### MSA-lite with REST only

장점:

- 서비스 분리 구조를 빠르게 만들 수 있다.
- Kafka 운영 부담이 없다.
...
- Kafka broker, topic, consumer 동작을 직접 다루는 범위가 줄어든다.
...
- Saga, Outbox, 멱등 Consumer를 주문 흐름의 핵심 설계 요소로 다룰 수 있다.
