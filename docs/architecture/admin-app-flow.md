...
- Read Model 기반 주문 Dashboard와 주문 ID/회원 ID/주문 상태/Saga 상태/쿠폰 코드 검색
- 쿠폰 사용 이력 조회와 상태 필터
...
|---|---|---|
| Dashboard | `GET /api/read-model/admin/orders` | Read Model 기반 조회 주문 수, 상태별 지표, 최근 주문, 취소 사유 요약과 `orderId`, `memberId`, `status`, `sagaStatus`, `couponCode` 조건 검색 |
...
- Dashboard는 Read Model projection만 조회하고 주문 처리 명령을 실행하지 않는다. 검색 조건은 선택 값만 반영하고 빈 값은 무시한다.
