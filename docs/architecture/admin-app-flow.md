...
Admin App은 StockRush 운영 시나리오를 시연하는 React 앱이다. 상품/재고 운영, 주문 Saga 상태, 서비스별 outbox 상태를 같은 앱에서 확인하고, due 상태의 pending 이벤트 재시도와 failed 이벤트 재처리 준비를 수동으로 실행할 수 있게 한다.
