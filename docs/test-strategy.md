...
- Gateway admin/customer authentication, role authorization, customer object-level authorization, Gateway-derived admin operator headers, and service-level audit rows for outbox retry/requeue, delayed payment cancel, product mutation, and stock mutation are now covered by integration tests. Keycloak-backed demo smoke is wired to acquire admin/customer tokens and pass bearer tokens into protected Gateway routes. App OIDC login remains a current security gap.
