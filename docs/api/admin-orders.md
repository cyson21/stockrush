...
| `sagaStatus` | no | none | `STARTED`, `INVENTORY_REQUESTED`, `PAYMENT_REQUESTED`, `PAYMENT_DELAYED`, `PAYMENT_CANCEL_REQUESTED`, `COMPLETED`, `FAILED` |
...
## Request Delayed Payment Cancellation
...
|---|---:|---|
| `Idempotency-Key` | yes | duplicate command guard key |
| `X-Correlation-Id` | no | trace key |

### Response Data

```json
{
  "orderId": "ord_20260513_002",
  "status": "CREATED",
  "sagaStatus": "PAYMENT_CANCEL_REQUESTED"
}
```

`CREATED` / `PAYMENT_DELAYED` orders are accepted and write `PaymentCancelRequested` to the order outbox. A retry after the order reaches `PAYMENT_CANCEL_REQUESTED` returns the same response shape without writing a duplicate command. Final order cancellation is completed after `PaymentCanceled` is consumed.
